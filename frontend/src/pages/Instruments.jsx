...
function Fret({ idx }){
  ...
  return (
    <button onClick={handle} className={`w-3 h-16 rounded-full transition-all relative ${active? 'bg-primary-container shadow-neu-inset' : 'bg-surface-container-highest shadow-[-4px_-4px_8px_#FFFFFF,4px_4px_8px_#D1CDC1]'}`}></button>
  )
}
...
export default function Instruments(){
  ...
  const instruments = [
    { name: 'Veena', icon: 'music_note', active: true },
    { name: 'Violin', icon: 'music_note', active: false },
    { name: 'Flute', icon: 'toys', active: false },
    { name: 'Mridangam', icon: 'queue_music', active: false }
  ]
          ...
          return (
            <button key={instrument.name} onClick={() => setSelectedInstrument(instrument.name)} className={`relative group flex flex-col items-center justify-center p-6 rounded-xl ${isActive ? 'bg-surface-container shadow-neu-inset' : 'bg-surface shadow-neu hover:shadow-neu-inset'}`}>
              <div className={`w-16 h-16 rounded-full mb-2 flex items-center justify-center ${isActive ? 'bg-primary-container shadow-neu' : 'bg-surface-container-high'}`}>
                <span className={`material-symbols-outlined ${isActive ? 'text-on-primary-container' : 'text-on-surface-variant'}`}>{instrument.icon}</span>
              </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-1 bg-surface-container rounded-2xl p-8 shadow-neu">
          <div className="relative w-full h-[420px] bg-surface rounded-xl shadow-[inset_8px_8px_16px_#D1CDC1,inset_-8px_-8px_16px_#FFFFFF] overflow-hidden flex items-center justify-center">
            ...
            <div className="absolute w-[80%] h-[120px] top-1/2 -translate-y-1/2 flex items-center justify-between px-6">
              {Array.from({length:8}).map((_,i)=>(<Fret key={i} idx={i} />))}
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-96 flex flex-col gap-6">
          <div className="bg-surface rounded-2xl p-6 shadow-neu">
            ...
            <p className="text-sm text-on-surface-variant">Record your interactions or play pre-recorded classic melodies to observe fret placement.</p>
            <div className="flex items-center justify-center gap-4 py-4">
              <button className="w-12 h-12 rounded-full bg-surface shadow-neu flex items-center justify-center"><span className="material-symbols-outlined">skip_previous</span></button>
              <button onClick={() => setIsPlaying((value) => !value)} className={`w-20 h-20 rounded-full ${isPlaying ? 'bg-primary' : 'bg-primary'} text-on-primary shadow-neu`}>
                <span className="material-symbols-outlined text-3xl">{isPlaying ? 'pause' : 'play_arrow'}</span>
              </button>
              <button className="w-12 h-12 rounded-full bg-surface shadow-neu flex items-center justify-center"><span className="material-symbols-outlined">skip_next</span></button>
            </div>
            <div className="w-full h-4 bg-surface-container rounded-full relative">
